...
| Recovery text-to-speech | OpenAI `tts-1`, voice `coral` | Audio Speech API, streamed playback | TTS-1 is the supported speed-optimized speech model. `gpt-4o-mini-tts` is not selected because its model page currently marks it deprecated. The application must tell the user that the voice is AI-generated. |
